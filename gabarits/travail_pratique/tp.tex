...
\usepackage{booktabs}          % Used for good looking tables
...
\newcommand{\siglecours}{420-12-MA}
\newcommand{\tptitle}{Travail pratique 1}
...
\begin{titlepage}
    ...
    {\Huge\bfseries {\siglecours} : {\tptitle}\par}
    \vspace{0.75cm}
    ...
    \begin{description}[leftmargin=0pt]
        \item[Cours] {\siglecours} Pédagogie hivernale.
        ...
        \item[Équipe] 2 à 3 bonhommes de neige. 
    \end{description}
...
\subsection{Lexique de la grille d'évaluation}
...
\begin{table}[ht]
  ...
  \caption{Lexique utilisé pour les grilles d'évaluation}
  \newcolumntype{R}[1]{>{\raggedright\arraybackslash}p{#1}}
  \fontsize{9pt}{11pt}\selectfont
  \begin{tabular}
    {R{2cm}R{2cm}R{2cm}R{2cm}R{2cm}R{2cm}}
    \toprule
    &
    \cellcolor{green!20}\textbf{Échelle} \newline Niveau 1 &
    \cellcolor{green!20!yellow!30}\textbf{Échelle} \newline Niveau 2 &
    \cellcolor{yellow!30}\textbf{Échelle} \newline Niveau 3 &
    \cellcolor{orange!20}\textbf{Échelle} \newline Niveau 4 &
    \cellcolor{red!15}\textbf{Échelle} \newline Niveau 5 \\
    \midrule
    \textbf{Critère} & & & & & \\
...
\subsection{Exemple de grille d'évaluation}

Un exemple de grille d'évaluation est présenté à la page suivante. Il s'agit
...
\begin{landscape} % Begin landscape mode
  \begin{table}[ht]
    ...
    \fontsize{9pt}{11pt}\selectfont
    \begin{tabular}
      {R{3.4cm}R{3.4cm}R{3.4cm}R{3.4cm}R{3.4cm}R{3.4cm}}
      \toprule
      &
      \cellcolor{green!20}\textbf{Excellent} \newline 100\% &
      \cellcolor{green!20!yellow!30}\textbf{Très bien} \newline $\geq$ 85\% &
      \cellcolor{yellow!30}\textbf{Bien} \newline $\geq$ 70\% &
      \cellcolor{orange!20}\textbf{Passable} \newline $\geq$ 60\% &
      \cellcolor{red!15}\textbf{Insuffisant} \newline $<$ 60\% \\
      \midrule
    ...
    Tous les critères, sauf un ou deux, sont décrits à l'aide d'un verbe  d'action mesurable à l'infinitif. &
    ...
    Tous les critères, sauf un ou deux, sont décrits à l'aide d'un verbe en lien avec le niveau taxonomique de la compétence. &
...
\begin{landscape} % Begin landscape mode
  \begin{table}[ht]
    ...
    \begin{tabular}
      ...
      \cellcolor{red!15}\textbf{Insuffisant} \newline $<$ 60\% \\
      \midrule
      \textbf{Préciser des indicateurs (25\%)} & & & & & \\
      Dimension précisée &
      ...
      Tous les indicateurs, sauf un ou deux, sont précis et communiquent clairement la
...
Une bonne rétroaction devrait aussi s'accompagner d'une série de commentaires
...
ayant obtenu le même niveau de performance dans la grille d'évaluation n'ont pas
...
\begin{landscape} % Begin landscape mode
  \begin{table}[ht]
    ...
    \begin{tabular}
      ...
      Verbe d'action mesurable à l'infinitif &
      Tous les critères sont décrits à l'aide d'un verbe d'action mesurable à l'infinitif. &
      \hl{Tous les critères, sauf un ou deux, sont décrits à l'aide d'un verbe  d'action mesurable à l'infinitif.} &
      La majorité des critères sont décrits à l'aide d'un verbe d'action mesurable  à l'infinitif. &
      La moitié des critères sont décrits à l'aide d'un verbe  d'action mesurable à l'infinitif. &
      Une minorité des critères sont décrits à l'aide d'un verbe  d'action mesurable à l'infinitif. \\
      ...
      Niveau taxonomique adéquat &
      Tous les critères sont décrits à l'aide d'un verbe en lien avec le niveau taxonomique de la compétence. &
      \hl{Tous les critères, sauf un ou deux, sont décrits à l'aide d'un verbe en lien avec le niveau taxonomique de la compétence.} &
      La majorité des critères sont décrits à l'aide d'un verbe en lien avec le niveau taxonomique de la compétence. &
      La moitié des critères sont décrits à l'aide d'un verbe en lien avec le niveau taxonomique de la compétence. &
      Une minorité des critères sont décrits à l'aide d'un verbe en lien avec le niveau taxonomique de la compétence. \\
...
\clearpage
\section{Distribuer la grille d'évaluation}
\label{sec:distribuez}
...
Ce gabarit est un exemple de grille d'évaluation et de travail pratique. Ajustez-le selon vos
besoins.
